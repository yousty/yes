...
RSpec.describe Yes::Auth::Generators::Principals::WriteResourceAccessGenerator do
  ...
  describe '#create' do
    ...
    it 'creates a migration file' do
      ...
      migration_files = Dir[File.join(migrate_dir, '*_create_auth_principals_write_resource_access.rb')]
    ...
    it 'generates correct migration content' do
      ...
      aggregate_failures do
        expect(content).to include('create_table :auth_principals_write_resource_accesses, id: :uuid')
        expect(content).to include('t.uuid :principal_id')
        expect(content).to include('t.uuid :role_id')
        expect(content).to include('t.string :context')
        ...
        expect(content).to include('add_index :auth_principals_write_resource_accesses, :principal_id')
        expect(content).to include('add_index :auth_principals_write_resource_accesses, :resource_id')
      end
    end
  end
end
