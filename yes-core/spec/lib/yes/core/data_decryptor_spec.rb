...
RSpec.describe Yes::Core::DataDecryptor do
  describe '#call' do
    subject { instance.call }

    let(:instance) { described_class.new(data:, schema:, repository:) }
    ...
    context 'when key is not found' do
      let(:failure) { DummyRepository::Failure.new('some failure') }
    ...
    context 'when data has not been encrypted (schema is nil)' do
      let(:instance) do
        described_class.new(data: decrypted_data, schema: nil, repository:)
      end
